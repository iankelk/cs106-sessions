...
The AIC-based stepwise search keeps `diabetes` in the model even though its Wald p-value is above 0.05. We require all predictors in the final logistic model to be significant at the 5% level, so we now refit a version of the stepwise-selected model that simply drops `diabetes` and keeps the remaining predictors.

```{r p1b-step-hl}
# Here we'll manually enforce that all predictors must be significant
# we drop diabetes from the stepwise-selected model and refit
glm_step_nodiab <- update(glm_step, . ~ . - diabetes)

summary(glm_step_nodiab)
...
knitr::kable(
  hl_summary,
  digits  = 4,
  caption = "Hosmer–Lemeshow goodness-of-fit test for the final reduced logistic model (without diabetes)"
)
```

**Conclusion:** The AIC-based stepwise procedure initially selected a model with six predictors: `pregnant`, `glucose`, `diastolic`, `bmi`, `diabetes`, and `age`. However, `diabetes` had a Wald p-value around 0.14 and therefore was not significant at the 5% level. To satisfy the requirement that all predictors in the final logistic model be significant, we refit the model without `diabetes`, yielding a five-predictor model with `pregnant`, `glucose`, `diastolic`, `bmi`, and `age`. The coefficient estimates change very little and the AIC increases only slightly (from 503.46 to 503.63), so we regard this as a reasonable trade-off between parsimony and fit. The Hosmer–Lemeshow test for this final reduced model produces a non-significant p-value, so we do not see strong evidence of lack of fit on the training data: the observed and expected counts across probability groups are reasonably consistent. As usual, the Hosmer–Lemeshow test should be interpreted alongside residual plots and other diagnostics rather than as a definitive verdict on model adequacy.
...
```{r p1e-test-preds}
# Predicted probabilities on the test data from the final reduced logistic model
